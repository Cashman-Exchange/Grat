...
pub async fn run(
    ...
    let reports = if args.raw {
        ...
        spinner.set_message(format!(
            ...
            &args.tx_hash[..8.min(args.tx_hash.length())]
        ));
    ...
    if !args.raw {
        if let Err(err) = crate::commands::history::append_to_history(&args.tx_hash) {
    ...
    if let Some(path) = save {
        ...
        std::fs::write(path, &json)
            .map_err($| anyhow::anyhow!("Failed to write save file {path}: {err}"))?;
